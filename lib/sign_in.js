...
var submitStatus = function () {
    ...
    data = (remark && toolStatus!= 0) ? data + '&remark=' + remark: data;
    ...
    xhr.onreadystatechange = function () {      //提交状态判断
        if (xhr.readyState == 4) {
            if (xhr.status == 200)
                NMFunc.route("sign_success.html");
            else
                NMFunc.route("sign_error.html");
        }
    };
    xhr.send(data);
};
...
//检查当天当片区是否提交过数据，回调结果成功提交数据，失败跳转页面
var getIsRepeat = function () {
    var token = document.querySelector("#token").value;
    var userBlock = {"6":1,"4":2,"5":3,"1":4,"3":5,"2":4};      //读取到的片区
    var xhr = new XMLHttpRequest();
    xhr.open("get","http://wts.sola.love/api/admin/toolscheck?token=" + encodeURIComponent(token)+"&block=" +
        encodeURIComponent(userBlock[area]),true);
    xhr.onreadystatechange = function () {      //提交状态判断
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                try {
                ...
                if (response.length == 0) {       //下次收到这种数先解析←_←
                    submitStatus();
                } else {
                    NMFunc.route("sign_repeat.html");
                }
            }
        }
    };
